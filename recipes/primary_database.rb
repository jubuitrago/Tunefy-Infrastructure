bash 'install_and_configure_primary_database' do
    code <<-EOH
        ...
        echo "host replication replicator REPLICA_DATABASE_IPX scram-sha-256" | sudo tee -a /etc/postgresql/16/main/pg_hba.conf > /dev/null
        sudo systemctl restart postgresql
        sudo -u postgres psql -c "CREATE DATABASE tunefy-database;"
